...
class FreeCADManualConverter:
    ...
    def extract_manual_links(self, base_url="https://wiki.freecad.org/Manual"):
        ...
        for a_tag in soup.find_all('a', href=True):
            href = a_tag.get('href')
            print(f"Found link: {href}")  # Debugging print statement
    ...
    def fetch_page(self, url):
        ...
        try:
            ...
            print(f"Debug: Fetched {response.url} with status code {response.status_code}") # Debug print
            
            if response.status_code != 200:
                print(f"Failed to fetch {url}. HTTP status code: {response.status_code}")
                return None
            
            # Check for anti-bot page content
            if "Making sure you&#39;re not a bot!" in response.text or "Protected by Anubis" in response.text:
                print(f"Warning: Detected anti-bot challenge page when fetching {url}. Cannot retrieve actual content.")
                return None
